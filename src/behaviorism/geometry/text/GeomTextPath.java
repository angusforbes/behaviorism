...
public class GeomTextPath extends GeomPoly
{
  ...
  public void draw()
  {
    ...
    renderInvisiblePickingBackground(offset);
  ...
  public void renderInvisiblePickingBackground(float offset)
  {
    GL2 gl = getGL();
...
//		Renderer.getInstance().resetPerspective3D();
        ...
        gl.glTranslatef(-w / 2f, -h / 2f, (float) (world_z)) ; //tx, ty, tz);

   // translate(-w / 2f, -h / 2f, (float) (world_z));

    //Scaling might be a bit wonky!!!!!!! it is for sure...
    if (parent != null)
    {
      //hmm deal with this wackiness...
      //parent.scale(gl);
//          gl.glTranslatef(parent.scaleAnchor.x, scaleAnchor.y, scaleAnchor.z);
...
//    gl.glTranslatef(-scaleAnchor.x, -scaleAnchor.y, -scaleAnchor.z);
    ...
    gl.glGetDoublev(GL_MODELVIEW_MATRIX, temp_mv, 0);
    gl.glGetDoublev(GL_PROJECTION_MATRIX, temp_pj, 0);
    gl.glGetIntegerv(GL_VIEWPORT, temp_vp, 0);
