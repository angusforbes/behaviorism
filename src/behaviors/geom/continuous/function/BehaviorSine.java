 ...
 * @author angus
 */
public class BehaviorSine extends BehaviorGeomContinuous
{
  float prevY = 0f;
  float amplitude = 1f;

  public static BehaviorSine sine(
    long startTime, 
    long lengthMS, 
    float min, float max)
  {
    return new BehaviorSine(
  ...
  public static BehaviorSine sine(
    ...
    float min, float max)
  {
    BehaviorSine bsw = new BehaviorSine(
      new ContinuousBehaviorBuilder(startTime, lengthMS).
      range(min, max).loop(LoopEnum.REVERSE) );
  ...
  float offset_val;
  public BehaviorSine(ContinuousBehaviorBuilder builder)
  {
    super(builder);
    this.offset_val = (float) ( (Math.sin(Math.toRadians(minvals[0] + (startPercent * ranges[0]) ))) );
...
/*
  ...
  */
  ...
  private float getValFromSineWave(float perc)
  {
